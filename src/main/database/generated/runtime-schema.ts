...
const RUNTIME_SCHEMA_TABLE_DDLS = [
  ...
  `CREATE TABLE IF NOT EXISTS "LiteratureExtraction" (
    "id" TEXT NOT NULL PRIMARY KEY,
    ...
    CONSTRAINT "LiteratureExtraction_state_check" CHECK ("state" IN ('queued', 'extracting', 'ready', 'partial', 'failed')),
    CONSTRAINT "LiteratureExtraction_textOrigin_check" CHECK ("textOrigin" IS NULL OR "textOrigin" IN ('embedded', 'ocr', 'hybrid')),
    CONSTRAINT "LiteratureExtraction_indexState_check" CHECK ("indexState" IN ('absent', 'queued', 'building', 'ready', 'failed')),
    CONSTRAINT "LiteratureExtraction_checksum_check" CHECK (length("documentChecksum") = 64 AND "documentChecksum" NOT GLOB '*[^0-9a-f]*' AND length("extractorFingerprint") = 64 AND "extractorFingerprint" NOT GLOB '*[^0-9a-f]*' AND ("contentChecksum" IS NULL OR (length("contentChecksum") = 64 AND "contentChecksum" NOT GLOB '*[^0-9a-f]*')) AND ("detectedMetadataChecksum" IS NULL OR (length("detectedMetadataChecksum") = 64 AND "detectedMetadataChecksum" NOT GLOB '*[^0-9a-f]*')) AND ("indexFingerprint" IS NULL OR (length("indexFingerprint") = 64 AND "indexFingerprint" NOT GLOB '*[^0-9a-f]*'))),
    CONSTRAINT "LiteratureExtraction_shape_check" CHECK ("mimeType" = 'application/pdf' AND "extractionSchemaVersion" >= 1 AND ("pageCount" IS NULL OR "pageCount" > 0) AND ("characterCount" IS NULL OR "characterCount" >= 0) AND ("indexedChunkCount" IS NULL OR "indexedChunkCount" >= 0)),
    CONSTRAINT "LiteratureExtraction_contentBundle_check" CHECK ((("contentStorageKey" IS NULL AND "contentChecksum" IS NULL) OR ("contentStorageKey" IS NOT NULL AND length(trim("contentStorageKey")) > 0 AND "contentChecksum" IS NOT NULL))),
    CONSTRAINT "LiteratureExtraction_metadataBundle_check" CHECK ((("detectedMetadataJson" IS NULL AND "detectedMetadataChecksum" IS NULL) OR ("detectedMetadataJson" IS NOT NULL AND json_valid("detectedMetadataJson") AND json_type("detectedMetadataJson") = 'object' AND "detectedMetadataChecksum" IS NOT NULL))),
    CONSTRAINT "LiteratureExtraction_indexBundle_check" CHECK ((("indexState" = 'absent' AND "indexFingerprint" IS NULL AND "indexedChunkCount" IS NULL) OR ("indexState" IN ('queued', 'building', 'failed') AND "indexFingerprint" IS NOT NULL) OR ("indexState" = 'ready' AND "indexFingerprint" IS NOT NULL AND "indexedChunkCount" IS NOT NULL))),
    CONSTRAINT "LiteratureExtraction_lifecycle_check" CHECK ((("state" IN ('queued', 'extracting') AND "errorCode" IS NULL) OR ("state" IN ('ready', 'partial') AND "textOrigin" IS NOT NULL AND "pageCount" IS NOT NULL AND "characterCount" IS NOT NULL AND "contentStorageKey" IS NOT NULL AND "errorCode" IS NULL) OR ("state" = 'failed' AND "errorCode" IS NOT NULL AND length(trim("errorCode")) > 0)))
);`,
  `CREATE TABLE IF NOT EXISTS "LiteratureEvidence" (
    "id" TEXT NOT NULL PRIMARY KEY,
    "projectId" TEXT NOT NULL,
    "sessionId" TEXT NOT NULL,
    ...
    CONSTRAINT "LiteratureEvidence_origin_check" CHECK ("origin" IN ('selection', 'retrieval')),
    CONSTRAINT "LiteratureEvidence_identity_check" CHECK (length(trim("sessionId")) > 0 AND length(trim("sourceMessageId")) > 0 AND "pageStart" >= 1 AND "pageEnd" >= "pageStart" AND "evidenceSchemaVersion" >= 1),
    CONSTRAINT "LiteratureEvidence_checksum_check" CHECK (length("documentChecksum") = 64 AND "documentChecksum" NOT GLOB '*[^0-9a-f]*' AND length("extractorFingerprint") = 64 AND "extractorFingerprint" NOT GLOB '*[^0-9a-f]*' AND length("evidenceChecksum") = 64 AND "evidenceChecksum" NOT GLOB '*[^0-9a-f]*'),
    CONSTRAINT "LiteratureEvidence_json_check" CHECK (json_valid("evidenceJson") AND json_type("evidenceJson") = 'object')
);`,
...
const RUNTIME_SCHEMA_INDEX_DDLS = [
  ...
  `CREATE INDEX IF NOT EXISTS "LiteratureExtraction_uploadVersionId_state_idx" ON "LiteratureExtraction"("uploadVersionId", "state");`,
  `CREATE INDEX IF NOT EXISTS "LiteratureExtraction_indexState_lastAccessedAt_idx" ON "LiteratureExtraction"("indexState", "lastAccessedAt");`,
  `CREATE UNIQUE INDEX IF NOT EXISTS "LiteratureExtraction_uploadVersionId_extractorFingerprint_extractionSchemaVersion_key" ON "LiteratureExtraction"("uploadVersionId", "extractorFingerprint", "extractionSchemaVersion");`,
  `CREATE INDEX IF NOT EXISTS "LiteratureEvidence_projectId_sessionId_sourceMessageId_idx" ON "LiteratureEvidence"("projectId", "sessionId", "sourceMessageId");`,
  `CREATE INDEX IF NOT EXISTS "LiteratureEvidence_uploadVersionId_idx" ON "LiteratureEvidence"("uploadVersionId");`,
  `CREATE INDEX IF NOT EXISTS "LiteratureEvidence_extractionId_idx" ON "LiteratureEvidence"("extractionId");`,
  `CREATE INDEX IF NOT EXISTS "LiteratureEvidence_documentChecksum_pageStart_idx" ON "LiteratureEvidence"("documentChecksum", "pageStart");`,
