...
describe('application database migrations', () => {
  ...
  it('records the runtime baseline once for a fresh database', async () => {
    ...
    await expect(
      ...
      from: null,
      to: '0018_literature_persistence'
    })
    ...
    await expect(migrateApplicationDatabase(client)).resolves.toEqual({
      adoptedLegacy: false,
      applied: [],
      from: '0018_literature_persistence',
      to: '0018_literature_persistence'
    })
  })
  ...
  it('keeps historical aggregate-only turn usage when adding exact model-call storage', async () => {
    ...
    await client.$executeRawUnsafe(
      `DELETE FROM "_open_science_migrations" WHERE "id" IN ('0015_session_model_call_usage', '0016_compute_job_sensitive_data_encryption', '0017_agent_memory_project_scope', '0018_literature_persistence')`
    )
  ...
  it('keeps a recovery snapshot when final verification rejects current schema drift', async () => {
    ...
    await expect(
      ...
      code: 'database_validation_failed',
      migrationId: '0018_literature_persistence'
    })
  ...
  it('creates recovery snapshots while retaining only the newest two for a ledger database', async () => {
    ...
    expect(backupEvents).toEqual([
      ...
      {
        migrationId: '0018_literature_persistence',
        path: `${databasePath}.before-0018_literature_persistence.backup`,
        reused: false
      }
    ])
    ...
    await expect(
      access(`${databasePath}.before-0018_literature_persistence.backup`)
    ).resolves.toBeUndefined()
    await expect(
  ...
  it('replays preview ownership migration when an adopted FK table contains orphan rows', async () => {
    ...
    await client.$executeRawUnsafe('DROP TABLE "LiteratureEvidence"')
    await client.$executeRawUnsafe('DROP TABLE "LiteratureExtraction"')
    await rm(join(storageRoot, 'open-science.db.before-0001_runtime_schema_baseline.backup'), {
      force: true
    })
  ...
  it('retains only the two newest restorable snapshots after adopting a legacy database', async () => {
    ...
    const literaturePersistenceBackupPath = `${databasePath}.before-0018_literature_persistence.backup`
    const backupEvents: unknown[] = []
    client = createProjectDbClient(storageRoot)
    await client.$executeRawUnsafe(`CREATE TABLE "Project" (
      "id" TEXT NOT NULL PRIMARY KEY,
    ...
    expect(backupEvents).toEqual([
      ...
      {
        migrationId: '0018_literature_persistence',
        path: literaturePersistenceBackupPath,
        reused: false
      }
    ])
    await expect(
      ...
      'open-science.db.before-0018_literature_persistence.backup'
    ])
    await expect(access(backupPath)).rejects.toMatchObject({ code: 'ENOENT' })
    ...
    await expect(access(literaturePersistenceBackupPath)).resolves.toBeUndefined()
    await expect(client.project.count()).resolves.toBe(1)

    const backupClient = new PrismaClient({
      datasources: { db: { url: `file:${literaturePersistenceBackupPath.replaceAll('\\', '/')}` } }
    })
    try {
      await expect(
