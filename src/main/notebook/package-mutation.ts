...
class NotebookPackageMutationOwner {
  ...
  async mutate({ target, mirror }: NotebookPackageMutationInput): Promise<InstallResult> {
    ...
    const managedCacheTransaction = journalTarget !== undefined
    const releaseWorkingCache = managedCacheTransaction
    ...
    try {
      ...
      await this.options.environmentOperations.runMutation(environmentName, async () => {
        ...
        await journal.begin({
          ...
          archivePublicationPending: managedCacheTransaction ? true : undefined
        })
        begun = true
        ...
        result = installResult
        if (managedCacheTransaction && archiveEvidenceIncomplete) {
          retainForRecovery = true
          throw new Error(
