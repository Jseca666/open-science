...
const useWorkspaceComposerController = ({
  ...
  const pdfReadingPosition = usePreviewWorkbenchStore((state) =>
    activeReadingBinding
      ? state.pdfReadingPositionByBindingId[activeReadingBinding.bindingId]
      : undefined
  )
  ...
  const reconcileReadingContextSources = useCallback(
    (requestedSources: SessionPdfContextSource[]): Promise<void> => {
      ...
      if (readingMutationPromiseRef.current) return readingMutationPromiseRef.current
  ...
  const captureReadingContextUndo = useCallback((): void => {
  ...
  }, [captureUndo, clearHistory, clearPastedTextUndo, markChanged])
  ...
  const linkReadingContext = useCallback(
    async (source: SessionPdfContextSource): Promise<void> => {
      ...
      if (
        ...
        return
      captureReadingContextUndo()
      await reconcileReadingContextSources([...current, source])
    },
    [activeSession, captureReadingContextUndo, reconcileReadingContextSources]
  )
