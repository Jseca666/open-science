...
- Plan approval uses the same content-bounded bottom panel shell and single-border embedded surface.
  ...
  leave the compact bottom entry for explicit navigation. Focus that entry when it first appears and
  return focus to the ordinary composer after the Plan resolves. On mobile, use the existing Preview
  Sheet as that single response surface and do not auto-open it.
- While Permission
...
- Left navigation: `w-48 shrink-0 border-r border-border bg-background p-3`, organized into labeled groups (for example Capabilities and Workspace). Each group has a `text-xs font-medium text-muted-foreground` heading over its rows. The single **Remote** destination belongs to Workspace after **Storage** and before **Usage**; do not create a singleton group for it.
